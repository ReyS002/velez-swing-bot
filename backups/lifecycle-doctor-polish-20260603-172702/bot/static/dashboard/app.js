...
async function refreshPositionDoctor() {
  try {
    ...
    lifecycleState = { ...currentLifecycleState(), doctor_positions: data.positions || [], summary: data.summary || currentLifecycleState().summary, timestamp: data.timestamp || new Date().toISOString() };
    dashboardState.lifecycle = lifecycleState;
    renderPanel();
