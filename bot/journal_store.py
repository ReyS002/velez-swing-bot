...
class JournalStore:
    ...
    def record_decision(self, snapshot: dict, order_payload: Optional[dict] = None, broker_response: Optional[dict] = None) -> None:
        ...
        # TEXT read back raw, so flatten lists to a readable comma-joined string.
        # Existing string values pass through unchanged.
    ...
    def set_watchlist_enabled(self, symbol: str, enabled: bool) -> bool:
        ...
        with self._connect() as db:
            cursor = db.execute(
                ...
                (1 if enabled else 0, _utc_now().isoformat(), cleaned),
            )
        return cursor.rowcount > 0
