...
export function mountWater(root, roomSnapshot) {
  ...
  let oceanRoom='';
  function setOceanMask(id){
    if(oceanRoom===id||!maskCtx)return;oceanRoom=id;canvas.dataset.oceanRoom=id;
    const cape=id==='cape';
    const shoreline=cape?[[1248,321],[1490,321],[1490,416],[1412,404],[1335,381],[1293,367],[1270,360],[1304,349],[1380,337],[1248,337]]:[[1292,306],[1510,306],[1510,462],[1455,471],[1340,455],[1293,443],[1257,439],[1170,420],[1100,402],[1065,391],[1150,382],[1258,365],[1258,324]];
    maskCtx.clearRect(0,0,1672,941);maskCtx.filter='blur(4px)';maskCtx.fillStyle='#fff';maskCtx.beginPath();shoreline.forEach(([x,y],i)=>i?maskCtx.lineTo(x,y):maskCtx.moveTo(x,y));maskCtx.closePath();maskCtx.fill();maskCtx.filter='none';
    if(cape){maskCtx.clearRect(1225,0,23,941);maskCtx.clearRect(1490,0,28,941);}else maskCtx.clearRect(1265,0,24,941);
  }
  ...
  function draw(now){raf=0;if(now-last<32){raf=requestAnimationFrame(draw);return;}const dt=Math.min((now-last)/1000||0,.05);last=now;elapsed+=dt;ctx.clearRect(0,0,1672,941);
    if(['pacific','cape'].includes(roomSnapshot().id)){setOceanMask(roomSnapshot().id);ocean(elapsed,root.dataset.theme==='night');}
  ...
  function sync(){
    ...
    if(active&&(root.dataset.rain==='on'||['pacific','cape'].includes(roomSnapshot().id))){last=performance.now();raf=requestAnimationFrame(draw);}else ctx.clearRect(0,0,1672,941);
  }
