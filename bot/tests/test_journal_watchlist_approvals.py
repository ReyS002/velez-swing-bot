...
def test_lifecycle_auto_claims_orphan_and_repairs_journal_stop(monkeypatch):
    monkeypatch.setenv("VELEZ_APPROVAL_API_TOKEN", "approval-token")
    broker = FakeBroker(
        positions=[
            {
    ...
    engine.journal.record_decision(
        {
            ...
            "alert_ref": "old-spy-entry",
            "status": "submitted",
            "reason": "submitted_to_alpaca_paper",
            "symbol": "SPY",
            "side": "buy",
            "play": "elephant_bar",
            "qty": 25,
    ...
    claimed = engine.claim_lifecycle_position("SPY", "old-spy-entry", "approval-token")
    repaired = engine.repair_lifecycle_stop("SPY", "approval-token")
    doctor = engine.lifecycle_doctor_payload()

    assert claimed["ok"] is True
    assert claimed["claim"]["alert_ref"] == "old-spy-entry"
    assert claimed["lifecycle"]["positions"][0]["linked_alert_ref"] == "old-spy-entry"
    assert repaired["ok"] is True
    assert broker.submitted[-1]["symbol"] == "SPY"
    assert broker.submitted[-1]["side"] == "sell"
    assert broker.submitted[-1]["type"] == "stop"
    assert broker.submitted[-1]["stop_price"] == "498.00"
    assert doctor["positions"][0]["linked_alert_ref"] == "old-spy-entry"


def test_lifecycle_preserves_fractional_crypto_qty_for_exit_actions(monkeypatch):
    monkeypatch.setenv("VELEZ_APPROVAL_API_TOKEN", "approval-token")
    broker = FakeBroker(
        positions=[
            {
    ...
    plan = engine.lifecycle_partial_plan()
    moved = engine.move_eligible_stops_to_breakeven("approval-token")

    assert plan["plans"][0]["qty"] == "0.125"
    assert plan["plans"][0]["options"][0]["qty"] == 0.03125
    assert moved["ok"] is True
    assert moved["moved_count"] == 1
    assert broker.submitted[-1]["qty"] == "0.125"
    assert broker.submitted[-1]["stop_price"] == "100000.00"


def test_lifecycle_reduce_position_submits_guarded_market_exit_and_verification(monkeypatch):
    monkeypatch.setenv("VELEZ_APPROVAL_API_TOKEN", "approval-token")
    broker = FakeBroker(
        positions=[
            {
    ...
    result = engine.reduce_lifecycle_position("SPY", 0.5, "approval-token")

    assert result["ok"] is True
    assert result["qty"] == "50"
    assert broker.canceled == ["spy-stop"]
    assert broker.submitted[-1]["symbol"] == "SPY"
    assert broker.submitted[-1]["side"] == "sell"
    assert broker.submitted[-1]["type"] == "market"
    assert result["verification"]["ok"] is True
    assert "scanner_reopen" in result["verification"]


def test_lifecycle_reduction_plan_blocks_scanner_reopen_at_full_exposure(monkeypatch):
    broker = FakeBroker(
        positions=[
            {"symbol": "SPY", "qty": "10", "side": "long", "avg_entry_price": "500", "current_price": "501"},
            {"symbol": "QQQ", "qty": "5", "side": "long", "avg_entry_price": "450", "current_price": "451"},
            {"symbol": "IWM", "qty": "7", "side": "long", "avg_entry_price": "220", "current_price": "221"},
        ],
        orders=[
            {"id": "s1", "symbol": "SPY", "side": "sell", "type": "stop", "qty": "10", "stop_price": "498"},
            {"id": "s2", "symbol": "QQQ", "side": "sell", "type": "stop", "qty": "5", "stop_price": "448"},
            {"id": "s3", "symbol": "IWM", "side": "sell", "type": "stop", "qty": "7", "stop_price": "218"},
        ],
    )
    cfg = config()
    cfg["scanner"] = {"enabled": True, "auto_submit": True, "pause_when_exposure_full": True}
    engine = TradingViewWebhookEngine(cfg, broker=broker)

    plan = engine.exposure_reduction_plan()

    assert plan["plan"]["suggestions"]
    assert plan["scanner_reopen"]["can_reopen"] is False
    assert "max_exposure_reached" in plan["scanner_reopen"]["blockers"]
...
def test_scanner_exposure_pause_sends_reduction_prompt(monkeypatch, tmp_path):
    ...
    cfg = config()
    cfg["webhook"]["execute_orders"] = True
    cfg["scanner"] = {"enabled": True, "auto_submit": True, "pause_when_exposure_full": True, "symbols": ["SPY"]}
    broker = FakeBroker(
        positions=[
            {"symbol": "SPY", "qty": "10", "side": "long", "avg_entry_price": "500", "current_price": "501"},
            {"symbol": "QQQ", "qty": "5", "side": "long", "avg_entry_price": "450", "current_price": "451"},
    ...
    content = notify_file.read_text()
    assert "exposure_reduction_prompt" in content


def test_scanner_auto_submit_pauses_when_lifecycle_has_critical_guardrail(monkeypatch):
    monkeypatch.setenv("VELEZ_EXECUTE_ORDERS", "true")
    monkeypatch.setenv("VELEZ_REQUIRE_ORDER_APPROVAL", "false")
    cfg = config()
    cfg["webhook"]["execute_orders"] = True
    ...
    assert status["pause"]["reason"] == "lifecycle_critical"
    assert status["skipped"] == ["all:lifecycle_critical"]
...
def test_daily_brief_telegram_notification_can_attach_audio(monkeypatch):
    monkeypatch.setenv("VELEZ_NOTIFY_TELEGRAM_BOT_TOKEN", "telegram-token")
    monkeypatch.setenv("VELEZ_NOTIFY_TELEGRAM_CHAT_ID", "telegram-chat")
    monkeypatch.setenv("VELEZ_NOTIFY_TELEGRAM_AUDIO_ENABLED", "true")
    posts = []
    ...
    def fake_post(url, json=None, data=None, files=None, timeout=None, headers=None):
        posts.append({"url": url, "json": json, "data": data, "files": files, "timeout": timeout})
        return FakeResponse()

    monkeypatch.setattr("bot.webhook_server.requests.post", fake_post)
    engine = TradingViewWebhookEngine(config(), broker=FakeBroker())
    engine.winston.synthesize_speech = lambda text: {
        "ok": True,
        "provider": "pockettts",
    ...
    result = engine.send_daily_brief_notification()

    assert result["ok"] is True
    assert result["telegram_audio_enabled"] is True
    assert len(posts) == 2
    assert posts[0]["url"].endswith("/sendMessage")
    assert posts[0]["json"]["chat_id"] == "telegram-chat"
    assert "Trading Bull Daily Brief" in posts[0]["json"]["text"]
    assert posts[1]["url"].endswith("/sendAudio")
    assert posts[1]["data"]["chat_id"] == "telegram-chat"
    assert posts[1]["files"]["audio"][0] == "trading-bull-brief.mp3"
